...
- **Eyebrow + hero** (`.events-hero`): `.eyebrow` ("Penn MEDIATED") above the serif, accent-purple `.events-hero__title` ("Events") and a plain sans lede — same shape as `about`'s Mission Statement block and `team-leadership`'s hero.
...
- **Past events** (`.past-events` / `.event-card`): a two-column grid (stacks to one column under 900px) of `.event-card`s — a small red date tag, title, and body copy, matching the type scale of `grant-card`/`news-card` elsewhere in the design system. The Media Fragmentation card uses `.event-card__media` (a 16:9 tile showing the real event photo, linked out to the `@PennMediated` YouTube channel — same "image tile + card-arrow badge + hover lift" language as `about`'s school-block/center-block/partner-card tiles), followed by a `.event-card__links` row of bordered link containers (white fill, 1px `--c-red` border, hover lift — same tile language, just pill-sized and with a brand-red outline instead of the tiles' usual hairline gray) rather than free-floating underlined text. The kickoff card uses `.event-card__photo-grid` for its two captioned photos.
...
- **`overflow-anchor: none`** (on `html`/`body`): same fix as `team-leadership`, for the same reason — prevents the browser's scroll-anchoring from nudging the viewport (and cropping the eyebrow) when web fonts swap in a beat after first paint.
